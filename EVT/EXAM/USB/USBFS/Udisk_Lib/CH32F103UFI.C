...
//uint8_t CtrlGetDeviceDescr( uint8_t *DataBuf )
//{
//    uint8_t ret;
//#if DEF_USB_PORT_FS_EN
//    ret = USBFSH_GetDeviceDescr( &RootHubDev[ DEF_USB_PORT_HS ].bEp0MaxPks, DataBuf );
//#elif DEF_USB_PORT_HS_EN
//    ret = USBHSH_GetDeviceDescr( &RootHubDev[ DEF_USB_PORT_HS ].bEp0MaxPks, DataBuf );
//#endif
//    return ret;
//}
...
uint8_t CtrlGetConfigDescr( uint8_t *DataBuf )
{
    ...
    uint8_t  ret;
#if DEF_USB_PORT_FS_EN
    ret = USBFSH_GetConfigDescr( RootHubDev[ DEF_USB_PORT_HS ].bEp0MaxPks, DataBuf, DEF_COM_BUF_LEN, &len );
#elif DEF_USB_PORT_HS_EN
    ret = USBHSH_GetConfigDescr( RootHubDev[ DEF_USB_PORT_HS ].bEp0MaxPks, DataBuf, DEF_COM_BUF_LEN, &len );
#endif
    return ret;
}
...
//	if ( use_external_interface ) return( extReadSector( CH103vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		pCBW -> mCBW_DataLen = (uint32_t)SectCount << CH103vSectorSizeB;  /* 数据传输长度 */ 
		pCBW -> mCBW_Flag = 0x80;
		pCBW -> mCBW_LUN = CH103vCurrentLun;
		pCBW -> mCBW_CB_Len = 10;
		pCBW -> mCBW_CB_Buf[ 0 ] = SPC_CMD_READ10;
		pCBW -> mCBW_CB_Buf[ 1 ] = 0x00;
		pCBW -> mCBW_CB_Buf[ 2 ] = (uint8_t)( CH103vLbaCurrent >> 24 );
		pCBW -> mCBW_CB_Buf[ 3 ] = (uint8_t)( CH103vLbaCurrent >> 16 );
		pCBW -> mCBW_CB_Buf[ 4 ] = (uint8_t)( CH103vLbaCurrent >> 8 );
		pCBW -> mCBW_CB_Buf[ 5 ] = (uint8_t)( CH103vLbaCurrent );
...
#ifdef	EN_DISK_WRITE
uint8_t CH103WriteSector( uint8_t SectCount, uint8_t *DataBuf )  /* 将缓冲区中的多个扇区的数据块写入磁盘 */
{
	uint8_t	retry;
...
uint8_t	CH103LibInit( void )  /* 初始化CH103程序库,操作成功返回0 */
{
...
#elif DEF_USB_PORT_HS_EN
    CH103vPacketSize = 512;  /* USB存储类设备的最大包长度:64@FS,512@HS/SS,由应用程序初始化,枚举U盘后如果是高速或者超速那么及时更新为512 */
...
//CH103vRootPort = 0;  /* USB主机选择(类似Root-hub根集线器选端口) */
	return( ERR_SUCCESS );
}
