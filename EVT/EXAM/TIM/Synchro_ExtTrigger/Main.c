 ...
 *slave mode routine:
 *TIM1_CH1(PA8)
 *This example demonstrates that when the TIM_CH1(PA8) pin detects a rising edge, TIM1 works differently in different slave modes.
 *RESET_MODE:TIM1->CNT Reset recount
 *GATED_MODE:When the PA8 pin inputs a low level, TIM1->CNT counts normally, otherwise it stops counting.
 *TRIGGER__MODE:TIM1->CNT keep counting.
 *
 */
