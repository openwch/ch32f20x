...
u8 HtmlBuffer[HTML_LEN];                              //Web page send buffer
...
const char Html_main[] = {
    ...
    "</a>\r\n"
    "</li>\r\n"
    "<!--Fourth line of the table of contents -->\r\n"
    "<li id=\"4\" >\r\n"
...
void Init_Para_Tab(void)
{
    ...
    strcpy(Para_Login[0].value, (char *)Login_CfgBuf.user);
    printf("__AUSE = %s\n", Para_Login[0].value);

    Para_Login[1].para = "__APAS";
    strcpy(Para_Login[1].value, (char *)Login_CfgBuf.pass);
    printf("__APAS = %s\n", Para_Login[1].value);
}
 ...
 * @return  none
 */
void Data_Send(uint8_t id, uint8_t *dataptr, uint32_t datalen)
{
    u32 len, totallen;
    uint8_t *p, timeout = 255;
    ...
    while(1){
		...
		if(--timeout == 0) break;
        if(totallen) 											//If the data is not sent, continue to send
		{
			Delay_Ms(1);
			continue; 
		}                                 
    ...
    while(reqnum){
        ...
        switch (http_request.METHOD)
        {
            ...
            case METHOD_POST:                                       //'post' request
                ...
                MakeHttpResponse((char *)httpweb, http_request.TYPE, pagelen);
