...
uint8_t IPMask[4];                                                   //subnet mask

uint8_t SocketId, SocketIdForListen;
uint8_t RecvBuffer[RECE_BUF_LEN], HTTPDataBuffer[RECE_BUF_LEN];
uint8_t SocketRecvBuf[WCHNET_MAX_SOCKET_NUM][RECE_BUF_LEN];          //socket receive buffer
uint16_t DESPORT, SRCPORT;                                           //port
/*********************************************************************
 * @fn      mStopIfError
 *
 ...
 * @return  none
 */
void WCHNET_CreateCfgSocket(uint8_t mode, uint8_t *Desip, u16 Desport, u16 Srcport)
{
    uint8_t i;
    SOCK_INF TmpSocketInf;
...
int main(void)
{
    ...
    /*After the button(PA6) is pressed, initialize
