...
void LDO_Deal(void)
{
    ...
    {
        if((RCC->APB1PCENR & ((uint32_t)(1 << 27))) == RESET) 
        {
            RCC->APB1PCENR |= ((uint32_t)(1 << 27));
            REGPSC |= ((uint32_t)(1 << 27));
        } 
        if((PWR->CTLR & ((uint32_t)(1 << 8))) == RESET)
        {
            PWR->CTLR |= ((uint32_t)(1 << 8));
            REGPSC |= ((uint32_t)(1 << 26));
        } 
        if((EXTI->EVENR & ((uint32_t)(1 << 17))) == RESET)
        {
            EXTI->EVENR |= ((uint32_t)(1 << 17));
            REGPSC |= ((uint32_t)(1 << 25));
        } 
        if((EXTI->RTENR & ((uint32_t)(1 << 17))) == RESET)
        {
            EXTI->RTENR |= ((uint32_t)(1 << 17));
            REGPSC |= ((uint32_t)(1 << 24));
        } 

        if((RCC->BDCTLR & ((uint32_t)(1 << 15))) == RESET)
        { 
            if((RCC->RSTSCKR & (1 << 1)) == RESET) REGPSC |= ((uint32_t)(1 << 30));
            ...
            REGPSC |= ((uint32_t)(1 << 29));
        }
    }
    REGPSC |= ((uint32_t)(0xF << 20));
...
void LDO_Restore(void)
{
    ...
    if((REGPSC & 0xF00000) == (uint32_t)0xF00000)
    {
        ...
        if((((*(vu32*)0x4002203C >> 24) & 0xFF) != 0xAB))
        {
            ...
            REGPSC |= (uint32_t)(((RTC->PSCRH & 0xF) << 16) | RTC->PSCRL);
            REGALR  = (uint32_t)(RTC->ALRMH << 16 | RTC->ALRML);
            do{
                high1a = RTC->CNTH;
                high1b = RTC->CNTH;
            }while(high1a != high1b);
...
{
#if((STOP_Mode == STOP_LowPower_Mode) && !defined(CH32F20x_D8) && !defined(CH32F20x_D8C))
