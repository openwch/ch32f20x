...
{
    if (ancs_c.state & (1 << ANCS_BT_CONNECTED) == 0) {
    ...
    if (ancs_c.state & (1 << ANCS_BT_DISCOVERY_COMPLETE) == 0) {
      ...
      Ancs_handleNotification(pMsg);
        ...
        ancs_c.state |= 1 << ANCS_CP_WRITE_PENDING;
        if (write_cb) {
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    else
    {
        ...
        ancs_c.state |= 1 << ANCS_BT_CONNECTED;
      ...
      case DISC_ANCS_CCCD:
        {
              ...
              (pMsg->method == ATT_ERROR_RSP))
          {
            ...
            else
            {
              ...
              ancs_c.state |= 1 << ANCS_BT_DISCOVERY_COMPLETE;
